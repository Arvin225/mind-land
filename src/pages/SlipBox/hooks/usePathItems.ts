...
function usePathItems() {
    const allCardPathItem: ItemType = { title: '全部卡片', href: undefined, onClick: e => e.preventDefault() }

    const [pathItems, setPathItems] = useState<ItemType[]>([allCardPathItem])
    const buildPathItems = (tagId: number | null, tagName?: string) => {
        ...
        if (splitNames.length > 1) {
            if (splitNames.length > 2) {
                ...
                pathItems.push({ title: '...', href: undefined, onClick: e => e.preventDefault() })
            } else {
                // 否则显示标签名 //todo 点击查询，携带标签名，处理点击时转化为id？
                pathItems.push({ title: splitNames[0], href: undefined, onClick: e => e.preventDefault() })
